...
def batch_generator_text_bart(batch):
    """
        Generates the batch for the TextModel. It pads the sequences up to the lenght of the longest sequence in the batch and
        group them by their key.
    ...
    for data in batch:
        max_len = max(max_len, len(data[0]['input_ids']))
        max_len_motion = max(max_len_motion, len(data[1]['input_ids']))
    ...
    keys = batch[0][0].keys()
    texts = {key: torch.stack([data[0][key] for data in batch]) for key in keys}
    motions = {key: torch.stack([data[1][key] for data in batch]) for key in keys}
    
    return texts, motions, torch.FloatTensor([b[2] for b in batch])
...
def batch_generator_mult_bart(batch):
    ...
    for data in batch:
        max_len = max(max_len, len(data[0]['input_ids']))
        max_len_motion = max(max_len_motion, len(data[1]['labels']))
    ...
    keys_motion = batch[0][1].keys()
    texts = {key: torch.stack([data[0][key] for data in batch]) for key in keys_text}
    motions = {key: torch.stack([data[1][key] for data in batch]) for key in keys_motion}
    
    return texts, motions, torch.cat([b[2] for b in batch], dim = 0), torch.FloatTensor([b[3] for b in batch])
